...
public class JogadorHumano extends Jogador{
    ...
    private Pokemon escolheNovoPokemon() {
        ...
        if(escolhido > getListaPokemons().size()) {
            DefaultOutput.showMessage("N�mero de pokémon inv�lido");
            escolhido = 0;
        }
        ...
        if(getListaPokemons().get(escolhido).getStatus().contains(StatusPrimario.FAINTED)) {
            ...
            DefaultOutput.showMessage(String.format("pokémon %s está morto, o pr�ximo vivo ser� escolhido.", getListaPokemons().get(escolhido)));
